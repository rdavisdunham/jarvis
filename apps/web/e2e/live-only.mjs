...
const token = process.env.JARVIS_LIVE_TEST_TOKEN;
const wav = readFileSync(new URL("tests/fixtures/live-create-task.wav", root)).toString(
  "base64",
);
...
try {
  await page.goto(process.env.JARVIS_LIVE_TEST_URL);
  ...
  await page.screenshot({
    path: new URL(".runtime/planner-live.png", root).pathname,
    fullPage: true,
  });
  await page
  ...
  await page.screenshot({
    path: new URL(".runtime/planner-voice-dock-mobile.png", root).pathname,
    fullPage: true,
  });
  ...
  if (evidence.idleStopAfterCheckSeconds < 7)
    throw new Error("Voice timed out too early");
  ...
  evidence.restartLive = true;
  await page.getByRole("button", { name: "End voice", exact: true }).click();
  await expect
    .poll(
      () =>
        page.evaluate(() =>
          window.__live.peers.every((p) => p.connectionState === "closed"),
        ),
      { timeout: 20000 },
    )
    .toBe(true);
  evidence.peersClosed = true;
  evidence.result =
  ...
  await browser.close();
  writeFileSync(
    new URL(".runtime/planner-live-evidence.json", root),
