...
test("ends after thirty quiet seconds, and speech near the deadline resets it", () => {
  const idle = new VoiceIdle();
  idle.start(0);
  expect(idle.expired(29999)).toBe(false);
  idle.touch(29999);
  expect(idle.expired(30000)).toBe(false);
  expect(idle.expired(59999)).toBe(true);
});
test("never expires during speech or task work and gives a full window afterwards", () => {
  ...
  expect(idle.expired(149999)).toBe(false);
  expect(idle.expired(150000)).toBe(true);
});
