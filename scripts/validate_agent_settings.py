"""Exercise model Settings in a disposable database with synthetic credentials."""
...
def main():
    ...
    name = "jarvis_agent_test_" + uuid4().hex
    ...
    try:
        ...
        os.environ.update(
            ...
            JARVIS_OWNER_TOKEN="agent-fixture",
            JARVIS_ORIGIN=base,
        ...
        with tempfile.TemporaryFile() as log:
            ...
            subprocess.run(
                ["node", "e2e/agent-settings.mjs"],
                cwd=ROOT / "apps" / "web",
                env={**os.environ, "JARVIS_AGENT_TEST_URL": base},
                check=True,
            )
    finally:
        if server:
